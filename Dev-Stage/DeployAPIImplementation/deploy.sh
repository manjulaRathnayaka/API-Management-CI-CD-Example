...
appHashId=$(curl -b cookies -X POST $SERVER_URL/appmgt/site/blocks/application/application.jag -F action=getApplication -F applicationName=HelloService |jq -r .hashId)

curl -v -b cookies -X POST $SERVER_URL/appmgt/site/blocks/application/application.jag -F action=deleteApplication -F applicationKey=$appHashId

curl -v -b cookies -X POST $SERVER_URL/appmgt/site/blocks/application/application.jag -F action=createApplication -F applicationName=HelloService -F applicationDescription=HelloService -F runtime=26 -F appTypeName=mss -F applicationRevision=1.0.0 -F uploadedFileName=Hello-Service-0.1-SNAPSHOT.jar -F runtimeProperties=[] -F tags=[] -F fileupload=@/home/travis/build/manjulaRathnayaka/API-Management-CI-CD-Example/APIImplementation/Hello-Service/target/Hello-Service-0.1-SNAPSHOT.jar -F isFileAttached=true -F conSpec=2 -F isNewVersion=false -F appCreationMethod=default -F setDefaultVersion=true
